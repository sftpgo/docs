...
:warning: The home directory is reached by resolving its path, so the directories that lead to it decide where a session lands. A redirection placed on that path — a symbolic link, a Windows junction, a bind mount — is followed, and the session is served from wherever it leads, with the privileges of the account SFTPGo runs under. Keep the home directory and every directory above it under administrative control and writable only by trusted accounts: provision them under a base directory reserved for them, as `users_base_dir` does, and give clients write access below their home rather than around it. This matters most where clients also hold local accounts on the host. The same applies to the mapped path of a [virtual folder](virtual-folders.md).
