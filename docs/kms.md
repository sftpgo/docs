...
- `master_key`, defines the master encryption key as string.
- `master_key_path` defines the absolute path to a file containing the master encryption key. If not empty, it takes precedence over `master_key`. This could be, for example, a docker secret or a file protected with filesystem level permissions.
...
:information_source: Setting a master key is recommended for every deployment: the master key lives outside the data provider, so the stored secrets stay confidential even if a copy of the data provider leaks. The master key is key material: supply a high-entropy value generated by a cryptographically secure random generator (for example `openssl rand -base64 32`), not a memorable passphrase. A weak master key is exposed to offline brute-force if the data store is compromised.
