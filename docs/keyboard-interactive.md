---
description: "Configure keyboard-interactive SSH authentication in SFTPGo using external programs or HTTP hooks for custom challenge-response flows."
---
...
- `auth_result`, integer. Set this field to 1 to indicate successful authentication. 0 is ignored. Set it to `2` along with a message in the `instruction` field to send the specified text and disconnect the client. Any other value means authentication error. If this field is found and it is different from 0 then SFTPGo will not read any other questions from the external program, and it will finalize the authentication.
