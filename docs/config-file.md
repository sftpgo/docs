...
- `token_validation`, integer. Define how to validate JWT tokens, cookies and CSRF tokens. By default a token must be used by the same IP for which it was issued, set to `1` to disable this requirement. Set to `2` to invalidate admin and user tokens issued before the last update. Flags can be combined. Default: `0`.
