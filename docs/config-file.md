...
| `secret_min_entropy` | float | `80` | Minimum entropy required for plain-text data-at-rest encryption secrets: the CryptFs passphrase and the S3 SSE-C key. These secrets are key material and must be random, not a memorable password. The value is a heuristic estimate (character-set and length based, like password-strength estimators), used to reject weak secrets — it is **not** a measure of cryptographic strength, so always supply key material generated by a CSPRNG. Any value is accepted (raise it to push harder toward random key material). The check runs when the secret is submitted in plain text (create, change, REST API, Terraform, `loaddata`), so secrets that are already stored keep working when this value is raised. Set to `0` to disable. See [Data At Rest Encryption](dare.md#passphrase-strength). |
