...
| `rename_mode` | integer | `0` | `0`: renaming non-empty directories is not allowed for cloud storage providers (S3, GCS, Azure Blob). `1`: enable recursive renames for these providers. Recursive renames may be slow and are not atomic; partial renames and incorrect quota updates are possible on error. At `1` the entries of a renamed directory are authorized individually, so a rename stops after moving part of the tree when one of them is refused. On the local filesystem a rename between the user's root filesystem and a virtual folder, or between two virtual folders, is also performed as a copy followed by a delete: see [Renaming across folders](virtual-folders.md#renaming-across-folders). |
...
Within that area, on these backends, clients holding the `create_symlinks` permission can create symbolic links when `symlink_mode` enables creation for the backend; it is disabled by default (see below). Creation also requires that permission on both the link's directory and the directory that contains the link's target: for a link to a directory, that directory's parent. Most file operations **dereference** a symbolic link: reading, writing, and metadata changes act on the link's *target*, not on the link node. Deleting or renaming a link, by contrast, acts on the link node itself, not on its target. Per-directory permissions are evaluated at the path the client requests, so a link placed in one directory that points into another runs operations under the permissions of the link's directory while acting on the target. When the two directories are governed by different per-directory permissions, operating through the link can reach the target in ways the target's own directory would not allow.

Dereferencing a link never re-checks `create_symlinks`: any link already present on the storage is followed under the requesting path's permissions. Only *creation* through SFTPGo is gated, and because it requires `create_symlinks` on the directory that contains the target, the permission is **bidirectional**: granting it on a directory authorizes that directory both as a place to create links and as a place whose entries links may point to, its subdirectories and everything below them included. An entry below a directory that carries `create_symlinks` can therefore be made a link target from any other directory that also carries it, reached under that other directory's permissions. An entry whose directory does **not** grant it cannot be made a *new* link target through SFTPGo, but this does not retract links that already exist: one created while the permission was granted, or placed on the storage outside SFTPGo, stays followed regardless. Do not grant `create_symlinks` on a directory when it, or any directory below it, restricts download, upload, or overwrite; and because revoking it later leaves existing links in place, keep creation disabled (the default `symlink_mode`) for any backend whose clients do not need it.
...
:information_source: Confinement and the per-directory checks validate each path before the operation runs, so a symbolic link resolving outside the accessible area is rejected and a persistent link can never reach beyond it. Because validation and the operation are separate steps, what another session changes between the two is not re-checked: under highly concurrent access a crafted link could be momentarily followed, and an entry added to a directory while it is renamed moves with it. Symbolic-link creation is disabled by default, so a client cannot introduce a link to exploit this; for environments with strict isolation requirements, provision each user from an empty directory that contains no symbolic links, so that with none present and none creatable there is nothing to act on, and keep the same permissions and filters on the paths a directory can be renamed between.

Per-directory permissions, file pattern filters and share paths are evaluated on the path the client requests, so a redirection placed on the storage (a symbolic link, a bind mount, a hard link, a Windows junction) is served under the restrictions of the requested path: see [How path-based restrictions are evaluated](access-control.md#how-path-based-restrictions-are-evaluated).
...
| `combine_support` | integer | `0` | Set to `1` to enable the non-standard `COMB` FTP command. Supported for the local filesystem; for cloud backends it offers no advantage as it downloads partial files and re-uploads the combined result. Cloud backends natively support multipart uploads. |
...
| `tls_session_reuse` | integer | `0` | TLS session reuse policy. `0`: not checked (clients may or may not reuse sessions). `1`: passive data connections must reuse the TLS session of the control connection, for explicit FTPS. Clients have to use session tickets, and the mode is not available for implicit TLS. `2`: reuse disabled (not recommended for security reasons). |
...
| `password_provider` | string | | Defines how the password for new database connections is obtained, for credentials that rotate. Supported values for the `mysql` and `postgresql` drivers: `aws`, an AWS RDS/Aurora IAM authentication token is generated for each new connection using the default AWS credentials chain (the region must be resolvable, e.g. via the `AWS_REGION` environment variable; tokens are bound to a single endpoint, so a single database host is supported), `azure`, a Microsoft Entra ID access token is requested for each new connection using the default Azure credentials chain (set the `AZURE_CLIENT_ID` environment variable to select a user-assigned managed identity), `file`, the password is re-read from `password_file` for each new connection, so it can be rotated externally, e.g. by Vault Agent or a Kubernetes secrets provider (the rotation must replace the file atomically, via rename; an empty file is an error). The `file` value is also supported for the `cockroachdb` driver. The `aws` and `azure` values send authentication tokens as password and require an `sslmode` that guarantees TLS: `1`-`3` for `postgresql`, `1`-`2` for `mysql`; with a custom `connection_string` the TLS setup is owned by the operator. For AWS RDS/Aurora, server certificate verification requires `root_cert` set to the [RDS certificate bundle](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/UsingWithRDS.SSL.html), since RDS certificates are issued by Amazon private CAs. If empty, the password defined via `password`/`password_file` is used for the lifetime of the process. |
...
| `connection_string` | string | | Custom database connection string. If not empty, this is used instead of building one from the previous parameters. Leave empty for `bolt` and `memory`. For `sqlite`, supplying a custom string bypasses SFTPGo's defaults (`_foreign_keys=1`, `_journal_mode=WAL`, `_busy_timeout=5000`) — include any pragma you want explicitly. Keep `_foreign_keys=1`: the server checks it at startup and stops when foreign keys are off, since the schema relies on them to keep the references between users, groups, folders and roles consistent. |
...
| `client_ip_proxy_header` | string | | Allowed client IP proxy header (e.g., `X-Forwarded-For`, `X-Real-IP`). The header is read only from connections whose address is listed in `proxy_allowed`; otherwise the connection address is used. |
...
| `redirect_base_url` | string | | Base URL to redirect to after OpenID authentication. The suffix `/web/oidc/redirect` is appended automatically (including `web_root` if configured). Use the same address your users browse: the login is completed on the browser that started it. |
...
| `insecure_skip_signature_check` | boolean | `false` | :warning: Accepts the ID token without validating its signature. Supported for providers that sign ID tokens with the `none` algorithm. See [security settings](oidc.md#security-settings). |
...
| `rp_initiated_logout` | boolean | `false` | Redirects the browser to the provider's end session endpoint on logout, so the single sign-on session is terminated (OpenID Connect RP-Initiated Logout). The SFTPGo login page is sent as `post_logout_redirect_uri` and must be registered with the provider as an allowed post-logout redirect URI. |
| `query_userinfo` | boolean | `false` | Queries the provider's UserInfo endpoint after authentication and reads the user claims from both sources. Non-empty ID token claims take precedence over UserInfo claims with the same name, see [UserInfo claims](oidc.md#userinfo-claims). The UserInfo subject must match the ID token subject. |
| `require_verified_email` | boolean | `false` | Accepts only identities whose email address is verified by the provider: the `email_verified` claim must be set to `true` in the ID token or, with `query_userinfo` enabled, in the UserInfo response, see [Verified email](oidc.md#verified-email). |
...
The master key is key material: supply a high-entropy value generated by a CSPRNG, not a memorable passphrase. No check can confirm that a value is cryptographically random: the key configured at startup is accepted as-is (the stored secrets depend on it), and the entropy screen `convertsecrets` applies to a new key is the same heuristic used for the CryptFs passphrase, so it rejects only obviously weak values. The strength requirement is on you to meet. The master key is never stored alongside the ciphertext, so a weak one is exposed to offline brute-force if the data store is compromised. To change the master key later, keep the current key in the configuration, re-encrypt the stored secrets onto the new key with the [`convertsecrets`](cli.md#re-encrypting-the-stored-secrets) command, and only then update the configuration. Simply replacing the key in the configuration without running `convertsecrets` first makes the secrets encrypted with the previous key unrecoverable.
...
The same settings are available in the WebAdmin under **Server Manager > Configurations > SMTP**, where they apply without a restart.
...
| `auth_type` | integer | `0` | Authentication type. `0` = Plain, `1` = Login, `2` = CRAM-MD5, `3` = XOAUTH2, `4` = SCRAM-SHA-256, `5` = SCRAM-SHA-256-PLUS. SCRAM-SHA-256-PLUS binds the authentication to the TLS channel and therefore requires `encryption` set to `1` or `2`. :information_source: Channel binding uses `tls-exporter` ([RFC 9266](https://datatracker.ietf.org/doc/html/rfc9266)) on TLS 1.3 and `tls-unique` on TLS 1.2. Servers that advertise `SCRAM-SHA-256-PLUS` but implement only `tls-unique` will reject the TLS 1.3 handshake; use `SCRAM-SHA-256` with those servers. |
| `allow_unencrypted_auth` | boolean | `false` | Allow the Plain and Login authentication types over an unencrypted connection. These types send the credentials as is, so enable this setting only when the connection to the SMTP server is secured in another way, for example an SSH tunnel or a trusted private network. Connections to `localhost`, `127.0.0.1` and `::1` are always allowed to authenticate over an unencrypted channel. |
...
| `refresh_token` | string | empty | OAuth2 refresh token. The WebAdmin can obtain one from the SMTP configuration page: complete the authorization in the browser that started it. |
