...
:information_source: On the local filesystem, including its encrypted variant, paths are resolved within the accessible area as part of each operation, one component at a time: a symbolic link resolving outside of it makes the operation fail, and a link with an absolute target is never followed, wherever it points. On the SFTP backend the path is validated and then operated on in separate steps; see [Path confinement](sftpfs.md#path-confinement) for the residual limits that apply there.
...
:information_source: More generally, SFTPGo trusts the storage. What the storage reports — the structure it presents, the names it returns, the file types and the metadata it declares — is taken as authoritative, and the restrictions above are layered on top of it. Content written to the storage outside SFTPGo, by another tool or directly on the filesystem, is served as it is reported.

:information_source: A public share exposes the shared paths as a boundary of the same kind as the path-based restrictions above: it is evaluated on the virtual path requested through the share link. A symbolic link beneath a shared path is dereferenced, so its target is served through the share even when it sits elsewhere in the owner's tree.
...
| `password_file` | string | empty | Path to a file containing the database password. Can be absolute or relative to the config dir. If not empty, takes precedence over `password`. |
...
| `delayed_quota_update` | integer | `0` | Number of seconds to accumulate quota updates. Accumulating updates can reduce data provider queries when uploads are frequent. A scheduled quota update is recommended regardless: quota is accounting kept by SFTPGo in software and is not enforced by the kernel or by the storage, so the stored counters are approximate by construction — a process killed mid-transfer, files changed outside SFTPGo, or a bug in the accounting leave them off. Use the [event manager](eventmanager.md) to schedule periodic quota updates. `0` means immediate quota update. |
...
| `share_cookie_lifetime` | integer | `120` | Duration in minutes for public sharing cookies. A browser session for a share remains valid for this duration even after the share password changes; delete the share to revoke access immediately. Maximum allowed value: `720`. |
...
| `base_url` | string | empty | External base URL for generating public links (e.g., share access links), bypassing the default browser-based detection. |
...
Setting a master key is recommended. The master key is key material: supply a high-entropy value generated by a cryptographically secure random generator, not a memorable passphrase. See [Key Management Services](kms.md).
