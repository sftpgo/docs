...
**Note:**: you might consider to use SFTPGo as SFTP backend for [rclone](https://rclone.org/sftp/){:target="_blank"} instead of `rsync`, this way there are no limitations and `rclone` does not need to be installed on the server side since it uses the SFTP protocol.
