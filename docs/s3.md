---
description: "Use S3-compatible object storage as a backend in open-source SFTPGo: AWS S3, Wasabi, Backblaze B2, MinIO, and others. Multipart uploads, SSE, and IAM roles supported."
---
...
- Directories are not real objects: the entries are derived from the object keys, so an upload brings the path prefixes it needs into existence.
