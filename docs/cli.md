...
The same dump can be loaded without starting the server, and restored as a mirror of its contents, using `initprovider`. See [Loading a dump from the command line](data-provider.md#loading-a-dump-from-the-command-line).
