...
SFTPGo can be managed and used programmatically through its REST API. The API is divided into two distinct areas:
...
The REST API is secured using JSON Web Tokens (JWT) or API key authentication. Enabling HTTPS is strongly recommended, to protect data in transit. Client certificate authentication can also be configured in conjunction with JWT.

## Configuration

The REST API can be enabled or disabled through the `httpd` configuration section, on a per-binding basis.

## Documentation and Client Generation

The API documentation is available in [OpenAPI format](https://sftpgo.com/rest-api){:target="_blank"} and describes all endpoints, request parameters, and response formats.

The OpenAPI specification can also be used to generate client libraries or custom integrations. Tools such as [Swagger Codegen](https://github.com/swagger-api/swagger-codegen){:target="_blank"} and [OpenAPI Generator](https://openapi-generator.tech/){:target="_blank"} generate clients for languages such as Python, Java, and JavaScript, as well as bash scripts.

## API Key Authentication

API keys provide a long-lived alternative to JWT tokens for REST API authentication. They are intended for automated scripts, CI/CD pipelines, and service-to-service integrations where repeatedly obtaining short-lived tokens is impractical.
